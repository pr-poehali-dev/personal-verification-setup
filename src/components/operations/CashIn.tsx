...
function generateOkud0402008(account: Account, amount: number, employee: Employee): string {
  const date = new Date();
...
ОПЕРАЦИЯ: ПРИЁМ НАЛИЧНЫХ ДЕНЕГ (ВЗНОС)
─────────────────────────────────────────────────
Номер документа: ${generateId().toUpperCase()}
Дата составления: ${dateStr}

ВНОСИТ: ${account.clientName}
Счёт зачисления: ${account.accountNumber}
Сумма: ${formatMoney(amount)}
Сумма прописью: ${amount.toLocaleString('ru-RU')} рублей

Источник поступления: Взнос наличных денег клиентом
─────────────────────────────────────────────────
Принял операционист: ${employee.name}
Подпись: ___________________
Кассир: ___________________
Подпись: ___________________

Деньги внёс: _______________ / ${account.clientName}
Подпись: ___________________
Дата: ${dateStr}
─────────────────────────────────────────────────
...
export default function CashIn({ employee }: CashInProps) {
  ...
  const handleCountConfirm = () => {
    ...
    data.transactions.unshift({
      ...
      amount: sum,
      currency: 'RUB',
      accountTo: foundAccount!.accountNumber,
      ...
      description: 'Взнос наличных по приходному ордеру',
      okudCode: '0402008',
    });
    saveData(data);
    const link = document.createElement('a');
    link.href = generateOkud0402008(foundAccount!, sum, employee);
    link.download = `ОКУД_0402008_${Date.now()}.txt`;
    link.click();
    toast({ title: '✅ Операция выполнена', description: `Принято ${formatMoney(sum)} от ${foundAccount!.clientName}` });
    setStep('done');
  };
  ...
  return (
    <div className="p-6 max-w-2xl animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Взнос наличных</h1>
          <p className="text-muted-foreground text-xs">Форма ОКУД 0402008</p>
        </div>
      </div>
      ...
      {step === 'done' ? (
        <div className="sber-card p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Взнос принят</h2>
          <p className="text-muted-foreground mb-6">Документ ОКУД 0402008 скачан автоматически</p>
        ...
        <div className="sber-card p-8 text-center">
          <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center border-2 border-primary" style={{ background: 'hsla(145,63%,42%,0.1)' }}>
            <Icon name="Banknote" size={32} className="text-primary" />
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Пересчитайте деньги и положите в кассу</h2>
          <p className="text-muted-foreground mb-2">Сумма взноса:</p>
          ...
          <button onClick={handleCountConfirm} className="w-full py-3 rounded-lg text-white font-semibold" style={{ background: 'hsl(var(--primary))' }}>
            Деньги в кассе · Подтвердить и скачать документ
          </button>
        </div>
      ) : step === 'sms' ? (
        <div className="sber-card p-8">
          <h2 className="text-lg font-bold text-white mb-2">Подтверждение операции</h2>
          <p className="text-muted-foreground text-sm mb-6">SMS-код для верификации клиента</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="sber-card p-5 space-y-4">
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1.5 uppercase tracking-wide">Сумма взноса (руб.)</label>
              <input
                type="number"
                value={amount}
          ...
          >
            Принять взнос · Отправить SMS-код
          </button>
        </form>
      )}
    </div>
