          ...
          <div className="sber-card p-5 space-y-4">
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type || 'text'} value={form[f.key as keyof typeof form]} onChange={e => set(f.key, e.target.value)}
                  className="w-full bg-secondary border border-border rounded-lg py-2.5 px-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary"
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={checkAccount} className="px-4 py-2.5 rounded-lg text-sm text-white" style={{ background: 'hsl(var(--secondary))', border: '1px solid hsl(var(--border))' }}>
                  Проверить
                </button>
              </div>
              {accountNotFound && (
